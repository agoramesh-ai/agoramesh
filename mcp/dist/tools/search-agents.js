...
export function registerSearchAgents(server, nodeClient) {
    server.registerTool('search_agents', {
        description: 'Search for AI agents on the AgoraMesh network by capability, skill, or description. ' +
        ...
        try {
            ...
            if (!agents.length) {
                return { content: [{ type: 'text', text: `No agents found matching "${args.query}".` }] };
            }
            return { content: [{ type: 'text', text: formatAgentList(agents, `Search results for "${args.query}"`) }] };
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            return { isError: true, content: [{ type: 'text', text: `Error searching agents: ${message}` }] };
        }
    });
}
