//! Load Testing for AgoraMesh P2P Network
//!
...
async fn test_concurrent_discovery_registration() {
    ...
    let cards: Vec<_> = (0..CONCURRENT_OPERATIONS)
        .map(|i| create_test_card(&format!("did:agoramesh:base:concurrent-{}", i)))
        .collect();
...
async fn test_high_volume_discovery_search() {
    ...
    let cards: HashMap<String, CapabilityCard> = (0..HIGH_LOAD_OPERATIONS)
        .map(|i| {
            let card = create_test_card(&format!("did:agoramesh:base:search-{}", i));
            let did = card
                .agoramesh
                .as_ref()
                .map(|ext| ext.did.clone())
...
async fn test_concurrent_trust_cache_operations() {
    ...
    for i in 0..CONCURRENT_OPERATIONS {
        let cache = Arc::clone(&cache);
        let handle = tokio::spawn(async move {
            let did = format!("did:agoramesh:base:trust-write-{}", i);
            let info = TrustInfo {
                did: did.clone(),
    ...
    for worker_id in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            while Instant::now() < deadline {
                ...
                let did = format!("did:agoramesh:base:stress-{}-{}", worker_id, op_num % 100);
...
fn test_juror_registration_throughput() {
    ...
    for i in 0..HIGH_LOAD_OPERATIONS {
        pool.register_juror(
            format!("did:agoramesh:base:juror-throughput-{}", i),
...
fn test_session_creation_throughput() {
    ...
    for i in 0..100 {
        pool.register_juror(
            format!("did:agoramesh:base:session-juror-{}", i),
            500_000_000,
            vec![0],
        )
        .unwrap();
...
fn test_large_card_cache_memory() {
    ...
    for i in 0..10000 {
        let card = create_test_card(&format!("did:agoramesh:base:memory-test-{}", i));
        let did = card
            .agoramesh
            .as_ref()
            .map(|ext| ext.did.clone())
    ...
    let mut found = 0;
    for i in 0..10000 {
        let did = format!("did:agoramesh:base:memory-test-{}", i);
