...
describe('TrustClient', () => {
    ...
    beforeEach(() => {
        ...
        mockPublicClient = client.getPublicClient();
    ...
    describe('getTrustScore()', () => {
        it('should throw error when client not connected', async () => {
            ...
            await expect(disconnectedTrust.getTrustScore('did:agoramesh:base:agent1')).rejects.toThrow('Client is not connected');
        });
    ...
    describe('getTrustFromNode()', () => {
        it('should fetch trust score from node REST API', async () => {
            ...
            const score = await trust.getTrustFromNode('did:agoramesh:base:agent1', 'https://api.agoramesh.ai');
        ...
        it('should throw on API failure', async () => {
            ...
            await expect(trust.getTrustFromNode('did:agoramesh:base:unknown', 'https://api.agoramesh.ai')).rejects.toThrow('Failed to get trust from node');
        });
    });
    // ===========================================================================
    ...
    describe('error handling', () => {
        it('should throw when trustRegistry not configured', async () => {
            client.getContractAddresses.mockReturnValueOnce({});
            await expect(trust.getTrustScore('did:agoramesh:base:agent1')).rejects.toThrow('TrustRegistry address not configured');
        });
        it('should propagate contract errors', async () => {
            mockPublicClient.readContract.mockRejectedValueOnce(new Error('Contract call reverted'));
            await expect(trust.getTrustScore('did:agoramesh:base:agent1')).rejects.toThrow('Contract call reverted');
        });
    });
    // ===========================================================================
