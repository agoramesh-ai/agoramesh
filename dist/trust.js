 ...
 */
function basisPointsToScore(bps) {
...
export class TrustClient {
     ...
     * ```
     */
    async getTrustFromNode(did, nodeUrl) {
    ...
    async getTrustScore(did) {
        ...
        const [reputationScore, stakeScore, endorsementScore, compositeScore] = await publicClient.readContract({
    ...
    async getTrustDetails(did) {
        ...
        const [reputationScore, stakeScore, endorsementScore, compositeScore] = trustScores;
        // Calculate success rate
        const totalTx = trustData.totalTransactions;
        const successfulTx = trustData.successfulTransactions;
        const successRate = totalTx > 0n ? Number((successfulTx * 10000n) / totalTx) : 0;
        return {
            scores: {
                overall: basisPointsToScore(compositeScore),
                reputation: basisPointsToScore(reputationScore),
     ...
     */
    async requestWithdraw(did, amount) {
        const walletClient = this.client.getWalletClient();
        const publicClient = this.client.getPublicClient();
        const addresses = this.client.getContractAddresses();
        ...
        const unlockTime = BigInt(Math.floor(Date.now() / 1000)) + BigInt(7 * 24 * 60 * 60);
     ...
     * ```
     */
    async endorse(endorseeDid, message = '') {
        const walletClient = this.client.getWalletClient();
        const addresses = this.client.getContractAddresses();
        if (!walletClient) {
            throw new Error('Wallet not connected.');
     ...
     * @returns Transaction hash
     */
    async revokeEndorsement(endorseeDid) {
        const walletClient = this.client.getWalletClient();
        const addresses = this.client.getContractAddresses();
        if (!walletClient) {
            throw new Error('Wallet not connected.');
     ...
     */
    async meetsTrustRequirement(did, minTrust) {
     ...
     */
    calculateEscrowRequirement(trustScore, taskValue) {
