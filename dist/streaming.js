...
(function (StreamStatus) {
    StreamStatus[StreamStatus["NONE"] = 0] = "NONE";
    StreamStatus[StreamStatus["ACTIVE"] = 1] = "ACTIVE";
    StreamStatus[StreamStatus["PAUSED"] = 2] = "PAUSED";
    StreamStatus[StreamStatus["CANCELED"] = 3] = "CANCELED";
    StreamStatus[StreamStatus["COMPLETED"] = 4] = "COMPLETED";
})(StreamStatus || (StreamStatus = {}));
/**
 * Human-readable stream status names.
 */
export const StreamStatusNames = {
...
export class StreamingPaymentsClient {
     ...
     * @returns The new stream ID
     */
    async createStreamWithTimestamps(options) {
        const walletClient = this.client.getWalletClient();
        const publicClient = this.client.getPublicClient();
        const addresses = this.client.getContractAddresses();
     ...
     */
    async getWithdrawableAmount(streamId) {
        const publicClient = this.client.getPublicClient();
        if (!publicClient) {
            throw new Error('Client is not connected.');
        }
     ...
     */
    async getStreamedAmount(streamId) {
        const publicClient = this.client.getPublicClient();
        if (!publicClient) {
            throw new Error('Client is not connected.');
        }
     ...
     */
    async getStreamsBySender(senderDid) {
        const publicClient = this.client.getPublicClient();
        if (!publicClient) {
            throw new Error('Client is not connected.');
        }
     ...
     * @returns Array of stream IDs
     */
    async getStreamsByRecipient(recipientDid) {
        const publicClient = this.client.getPublicClient();
        if (!publicClient) {
            throw new Error('Client is not connected.');
        }
     ...
     */
    calculateRate(amount, durationSeconds) {
        const amountWei = parseUSDC(amount);
     ...
     */
    async isSender(streamId) {
        const stream = await this.getStream(streamId);
        const address = this.client.getAddress();
        return (address !== null && address.toLowerCase() === stream.sender.toLowerCase());
    }
    /**
     ...
     */
    async getTimeRemaining(streamId) {
        const stream = await this.getStream(streamId);
     ...
     */
    async getCancellationPreview(streamId) {
        const stream = await this.getStream(streamId);
        const duration = stream.endTime - stream.startTime;
        const elapsed = calculateElapsedTime(stream.startTime, stream.endTime);
        // Calculate streamed amount (what recipient has earned)
        const streamedAmount = duration > 0n ? (stream.depositAmount * elapsed) / duration : stream.depositAmount;
        // Recipient gets: streamed - already withdrawn
        const recipientAmount = streamedAmount > stream.withdrawnAmount
            ? streamedAmount - stream.withdrawnAmount
            : 0n;
        // Sender refund: deposit - streamed
        const senderRefund = stream.depositAmount > streamedAmount
        ...
        const canCancel = (isSender && stream.cancelableBySender) ||
     ...
     */
    async getStuckStreams(senderDid) {
    ...
    static WARNING_THRESHOLD_SECONDS = 300;
    /**
