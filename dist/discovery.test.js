 ...
 */
function sampleCapabilityCard(did) {
    return {
        id: did,
        name: 'Test Agent',
 ...
 */
function sampleDiscoveryResult(did) {
    return {
        did,
        name: 'Test Agent',
...
describe('DiscoveryClient', () => {
    ...
    describe('search()', () => {
        it('should throw error when node URL not configured', async () => {
            await expect(discovery.search('translate documents')).rejects.toThrow('Node URL not configured');
        });
        ...
        it('should throw error on API failure', async () => {
            ...
            await expect(discovery.search('translate')).rejects.toThrow('Discovery search failed: Internal Server Error');
        });
    });
    // ===========================================================================
    // searchByTags Tests
    ...
    describe('getCapabilityCard()', () => {
        it('should fetch from well-known URL for did:web', async () => {
            ...
            expect(global.fetch.mock.calls[0][0]).toBe('https://agent.example.com/.well-known/agent.json');
        });
        ...
        it('should try IPFS if DHT fails', async () => {
            ...
            client.getAgent.mockResolvedValueOnce({
                did: 'did:agoramesh:base:agent1',
        ...
        it('should return null if all sources fail', async () => {
            discovery.setNodeUrl('https://node.example.com');
            client.getAgent.mockResolvedValueOnce(null);
            global.fetch
    ...
    describe('announce()', () => {
        ...
        it('should POST capability card to agents endpoint', async () => {
            ...
            expect(JSON.parse(call[1].body)).toEqual(card);
        });
    ...
    describe('unannounce()', () => {
        it('should throw not yet supported error', async () => {
            discovery.setNodeUrl('https://node.example.com');
            await expect(discovery.unannounce('did:agoramesh:base:agent1')).rejects.toThrow('not yet supported');
        });
    });
    // ===========================================================================
