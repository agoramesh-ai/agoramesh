...
export class DiscoveryClient {
    ...
    async search(query, options = {}) {
        if (!this.nodeUrl) {
            throw new Error('Node URL not configured. Call setNodeUrl() or pass nodeUrl to constructor.');
        }
        const { minTrust = 0, maxPrice, tags = [], currency, limit = 20, offset = 0, } = options;
        ...
        const response = await fetch(`${this.nodeUrl}/agents/semantic?${params.toString()}`, {
            method: 'GET',
            headers: {
    ...
    async searchByTags(tags, options = {}) {
        ...
        const response = await fetch(`${this.nodeUrl}/agents?${params.toString()}`, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
            },
     ...
     */
    async fetchFromDHT(did) {
        if (!this.nodeUrl) {
            return null;
        }
        try {
            const response = await fetch(`${this.nodeUrl}/agents/${encodeURIComponent(did)}`, {
                method: 'GET',
                headers: {
                    Accept: 'application/json',
                },
     ...
     */
    async announce(card, adminToken) {
        if (!this.nodeUrl) {
            throw new Error('Node URL not configured.');
        }
        const headers = {
     ...
     */
    async unannounce(_did) {
        throw new Error('Agent removal is not yet supported by the node API. ' +
     ...
     */
    filterBySkills(results, skillIds) {
        return results.filter((result) => skillIds.every((skillId) => result.matchingSkills.some((skill) => skill.id === skillId)));
    }
    /**
     ...
     */
    async getRecommendations(query, requirements = {}) {
