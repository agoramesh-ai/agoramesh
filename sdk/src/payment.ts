...
export class PaymentClient {
   ...
   * ```
   */
  async createEscrow(options: CreateEscrowOptions): Promise<bigint> {
   ...
   * @returns The new escrow ID
   */
  async createAndFundEscrow(options: CreateEscrowOptions): Promise<bigint> {
   ...
   * @returns Transaction hash
   */
  async claimTimeout(escrowId: bigint): Promise<`0x${string}`> {
    const walletClient = this.client.getWalletClient();
    const addresses = this.client.getContractAddresses();
   ...
   */
  async canClaimTimeout(escrowId: bigint): Promise<boolean> {
    const escrow = await this.getEscrow(escrowId);
   ...
   */
  async getTimeUntilDeadline(escrowId: bigint): Promise<number> {
    const escrow = await this.getEscrow(escrowId);
   ...
   */
  async getEscrowAmount(escrowId: bigint): Promise<string> {
    const escrow = await this.getEscrow(escrowId);
   ...
   */
  async getEscrowStateName(escrowId: bigint): Promise<string> {
    const escrow = await this.getEscrow(escrowId);
   ...
   */
  async isClient(escrowId: bigint): Promise<boolean> {
    const escrow = await this.getEscrow(escrowId);
   ...
   */
  async isProvider(escrowId: bigint): Promise<boolean> {
    const escrow = await this.getEscrow(escrowId);
    const address = this.client.getAddress();
    return (
      address !== null &&
