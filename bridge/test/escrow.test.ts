 ...
 *
 * TDD tests for Bridge <-> AgoraMeshEscrow contract integration.
 */
...
describe('didToHash', () => {
  it('converts DID string to bytes32 hash', () => {
    const did = 'did:agoramesh:base:0x1234567890123456789012345678901234567890';
  ...
  it('generates deterministic hash for same DID', () => {
    const did = 'did:agoramesh:base:0xabc123';
  ...
  it('generates different hashes for different DIDs', () => {
    const hash1 = didToHash('did:agoramesh:base:agent1');
    const hash2 = didToHash('did:agoramesh:base:agent2');
